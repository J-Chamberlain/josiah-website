...
interface ApiResponse {
  ...
  regionEstimate: { intersectedBands: string[]; dominantBand: string | null };
  narrative: HistoryNarrative;
}

export default function ExplorerAlt({
  imageSrc,
  ...
  const [analysisState, setAnalysisState] = useState<'closed' | 'compact' | 'expanded'>('closed');
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Failed to generate history narrative.');
      ...
      startTransition(() => {
        ...
        setAnalysisState((s) => s === 'closed' ? 'compact' : s);
      });
    } catch (err) {
  ...
  return (
    <div className={`hex-layout ${analysisState === 'expanded' ? 'is-analysis-expanded' : ''}`}>

      {/* ── Left: chart fills full height ── */}
      <div className="hex-chart-pane">
        <ImageViewport
          imageSrc={imageSrc}
          imageAlt={imageAlt}
      ...
      {/* ── Right: heading + controls + collapsible analysis ── */}
      <aside className="hex-side-pane">

        <div className="hex-side-header">
          <p className="history-explorer__eyebrow">Interactive Prototype</p>
          <h1 className="hex-side-title">History Explorer</h1>
          <p className="hex-side-desc">
        ...
        <div className="hex-toolbar">
          ...
          <button
            type="button"
            className="history-explorer__analyze hex-analyze-btn"
            onClick={handleAnalyze}
            disabled={!normalizedSelection || loading}
          >
        ...
        <div className="hex-analysis">
          {analysisState === 'closed' ? (
            <button
              type="button"
              className="hex-analysis-show"
              onClick={() => setAnalysisState('compact')}
            >
              Selection Analysis ›
            </button>
          ) : (
            <NarrativePanel
              panelState={analysisState}
              onSetPanelState={setAnalysisState}
              loading={loading}
              error={error}
